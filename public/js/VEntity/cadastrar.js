...
function deleteAtributo(id) {
    if (confirm('Confirme para excluir este atributo?')) {
        var idTable = '#' + id;
        $(idTable).fadeOut('slow', function(){ $(idTable).remove(); });
    }
}
...
function adicionarAtributo() {
    if (confirm('Desejá adicionar um novo atributo')) {
        /* id do atributo */
        var idForm = '#div_atributo_'+key_atributos; 
        /* obter html do form */
        var htmlImputAtributo = htmlFormAtributo();
    ...
    var html = '<div class="grid fluid" style="padding: 10px;background: #F0EEEE; border: 1px solid #E0DDDD;" id="div_atributo_'+key_atributos+'">';
        ...
        html += '<div class="row">';
            html += '<div class="span12">';
                html += '<div class="toolbar transparent link place-right">';
                    html += '<a><button class="bt-frame fg-red deleteButton" style="margin-top: -20px;" onclick="deleteAtributo(\'div_atributo_'+key_atributos+'\')"><i class="icon-cancel"></i></button></a>';
                html += '</div>';
            html += '</div>';
        html += '</div>';
        ...
        html += '<div class="row">';
            ...
            html += '<div class="span4">';
                ...
                html += '<div class="input-control text" data-role="input-control">';
                    html += '<input type="text" name="dados['+key_atributos+'][colmap]" id="colmap_'+key_atributos+'" required="" />';
                    html += '<button class="btn-clear" tabindex="-1" type="button"></button>';
                html += '</div>';
            html += '</div>';
