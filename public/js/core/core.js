...
function AjaxSession(dados) {
    ...
    $.ajax({
        type: "POST",
        url: "index.php?c=Request&a=ajaxSession",
        async: false,
        data: {
            dados: dados
        },
...
function limparSelect2(id, mensagem) {

    var idObject = '#' + id;
    var objectFrom = 'div[id="s2id_' + id + '"] a:first';

    $(idObject).html('<option value=""></option>');
    $(objectFrom).html('<span class="select2-chosen">' + mensagem + '</span><abbr class="select2-search-choice-close"></abbr>');
...
function dialogFlatIframe(title, icon, url) {
    $.Dialog({
        ...
        onShow: function(_dialog) {
            var html = [
                '<iframe width="1024" height="480" src="' + url + '" frameborder="0" allowfullscreen></iframe>'
            ].join("");
